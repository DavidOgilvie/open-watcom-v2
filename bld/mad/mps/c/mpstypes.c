...
*  ========================================================================
*
* Description:  MIPS machine type handling.
*
****************************************************************************/
...
// NYI: Hardcoded to little endian, should be dynamic
#define TARG_ENDIAN         ME_LITTLE
...
mad_type_handle MADIMPENTRY( TypeForDIPType )( const dig_type_info *ti )
{
    ...
    case TK_INTEGER:
        switch( ti->size ) {
        case 1:
            return( ti->modifier == TM_SIGNED ? MIPST_CHAR : MIPST_UCHAR );
        case 2:
            return( ti->modifier == TM_SIGNED ? MIPST_SHORT : MIPST_USHORT );
        case 4:
            return( ti->modifier == TM_SIGNED ? MIPST_LONG : MIPST_ULONG );
        case 8:
            return( ti->modifier == TM_SIGNED ? MIPST_INT64 : MIPST_UINT64 );
        }
        break;
    case TK_REAL:
        switch( ti->size ) {
        case 4:         return( MIPST_FLOAT );
        case 8:         return( MIPST_DOUBLE );
        }
        break;
    case TK_CODE:
    case TK_ADDRESS:
