...
typedef int32_t status_array[256];
 ...
 */
static int8_t
getStateProp(const status_array *stateTable, int8_t stateProps[], int state) {
    const int32_t *row;
...
ucnv_MBCSLoad(UConverterSharedData *sharedData,
    ...
    if(mbcsTable->outputType==MBCS_OUTPUT_EXT_ONLY) {
        *pErrorCode=U_INVALID_TABLE_FORMAT;
        ...
        mbcsTable->stateTable=(const status_array *)(raw+headerLength*4);
...
static UBool
hasValidTrailBytes(const status_array *stateTable, uint8_t state) {
 ...
 */
static UBool
isSingleOrLead(const status_array *stateTable, uint8_t state, UBool isDBCSOnly, uint8_t b) {
    const int32_t *row=stateTable[state];
    int32_t entry=row[b];
