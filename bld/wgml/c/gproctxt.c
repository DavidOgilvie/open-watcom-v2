...
static void wgml_tabs( void )
{
    ...
    for( ; i < in_count; i++ ) {
        ...
        if( s_multi == NULL ) {
            ...
            pre_space = in_chars->x_address - (in_chars->prev->x_address + in_chars->prev->width);
        }
...
//    if( (sum_w <= 0) || (hor_end >= rm) || (line_width < 1) ) {
    ...
    if( input_cbs->fmflags & II_research && GlobFlags.lastpass ) {
...
void    process_line_full( text_line * a_line, bool justify )
{
    ...
    if( justify && GlobFlags.lastpass && !ProcFlags.literal
