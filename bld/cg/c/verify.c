...
/************************************************/
{
    ...
    if( n1->t.location == NO_LOCATION )
        return( false );
    if( n2->t.location == NO_LOCATION )
        return( false );
    base = DeAlias( n1 );
...
*/
{
    if( op->c.const_type == CONS_ABSOLUTE ) {
        ...
        case U2:
        case U4:
            if( CFIsI16( op->c.value ) && op->c.lo.int_value == 0 )
                return( true );
            break;
        case I1:
...
*/
{
    ...
    case V_OP2NEG:
        if( ( op2->c.const_type == CONS_ABSOLUTE )
         ...
         && ( op2->c.lo.int_value != 0x80000000 ) )
            return( true );
        break;
    case V_OP2ONE:
        if( op2->c.const_type == CONS_ABSOLUTE
         && op2->c.lo.int_value == 1 )
            return( true );
        break;
    case V_OP2POW2:
    ...
    case V_OP2ZERO:
        ...
        break;
    case V_OP2_FFFFFFFF:
        if( op2->c.lo.int_value == 0xFFFFFFFF )
            return( true );
        break;
    case V_SAME_LOCN:
         if( op1->n.class != N_TEMP )
            return( false );
         if( result->n.class != N_TEMP )
            return( false );
    ...
    case V_SWAP_GOOD:
        ...
        case N_CONSTANT:
            return( false );
        case N_TEMP:
            if( op2->t.temp_flags & CONST_TEMP )
                return( false );
            /* fall through */
        default:
            if( op1 != result )
