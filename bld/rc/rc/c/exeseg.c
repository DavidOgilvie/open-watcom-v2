...
/************************************************/
{
    ...
    ret = readSegTable( old_fp, head_offset + head->segment_off, oldseg );
    if( ret != RS_OK ) {
        *err_code = errno;
        return( ret );
    }
    ret = readSegTable( old_fp, head_offset + head->segment_off, tmpseg );
    *err_code = errno;
    return( ret );
 ...
 */
{
    ...
    if( cponeret == CPSEG_ERROR ) {
        ...
        if( RESSEEK( old_exe_info->fp, end_offset, SEEK_SET ) ) {
            ret = CPSEG_ERROR;
