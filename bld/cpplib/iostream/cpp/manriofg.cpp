...
// Handles "resetioflags" manipulator.
static std::ios &__resetiosflags( std::ios &strm, long flags ) {
    strm.unsetf( flags );
    return( strm );
}
...
namespace std {
  SMANIP_make( long, resetiosflags ) {
    // SMANIPlong resetiosflags( long __p )
    //      Set all format flags to those specified in "__p".
    return SMANIPlong( __resetiosflags, __p );
  }
}
#endif
...
#ifdef __SIMPLIFY__
namespace std {
  smanip<long> resetiosflags( long parm ) {
    return smanip<long>( __resetiosflags, parm );
  }
}
#else
#pragma initialize 22; // just after the rest of the C++ library
namespace std {
  _WPRTLINK sapp<long> _HUGEDATA resetiosflags( __resetiosflags );
}
#endif
