...
extern "C"
_WPRTLINK
void * CPPLIB( ctor_varray )(   // CALL CONSTRUCTORS FOR ARRAY ELEMENTS
    void *array,                // - array
    unsigned count,             // - number of elements
    RT_TYPE_SIG sig )           // - signature of array
{
    unsigned size;              // - size of one element
    pFUNctorV ctor;             // - constructor for an element
                                // - array-ctor control
    _RTARCTOR ar_ctor( 0, sig, array );

    ctor = sig->clss_v.ctor;
    size = sig->clss_v.size;
    while( ar_ctor._count < count ) {
        (*ctor)( array, CTOR_NULL );
